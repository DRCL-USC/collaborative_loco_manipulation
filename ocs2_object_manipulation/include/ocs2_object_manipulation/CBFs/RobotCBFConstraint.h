...
namespace ocs2
{
  namespace object_manipulation
  {
    ...
    {
    public:
      RobotCBFConstraint(std::shared_ptr<Obstacles> obstacles, scalar_array_t radius_array, scalar_t alpha, scalar_array_t init_yaw)
          : StateInputConstraint(ConstraintOrder::Linear), obstacles_(obstacles), radius_array_(radius_array), alpha_(alpha), init_yaw_(init_yaw)
      {};

      ~RobotCBFConstraint() override = default;
      RobotCBFConstraint *clone() const override { return new RobotCBFConstraint(*this); }

      size_t getNumConstraints(scalar_t time) const override { return AGENT_COUNT * obstacles_->getObstacles().size(); };
